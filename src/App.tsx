...
export default function App() {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#F7F8FA] overflow-hidden">
      ...
      {/* Mobile Bottom Nav */}
      <div className="lg:hidden flex items-center justify-around h-14 bg-[#FFFFFF] border-t border-[#E5E7EB] flex-shrink-0">
        <button
          onClick={() => setMobilePanel(mobilePanel === 'left' ? 'none' : 'left')}
          className={`flex flex-col items-center gap-0.5 text-xs font-medium transition-colors ${
            mobilePanel === 'left' ? 'text-[#18181B]' : 'text-[#71717A]'
          }`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 text-xs font-medium transition-colors ${
            mobilePanel === 'none' ? 'text-[#18181B]' : 'text-[#71717A]'
          }`}
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
            <rect x="3" y="3" width="18" height="18" rx="2" />
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 text-xs font-medium transition-colors ${
            mobilePanel === 'right' ? 'text-[#18181B]' : 'text-[#71717A]'
          }`}
        >
